...
export interface IStorage {
  ...
  updateThought(id: string, thought: Partial<InsertThought>, tagNames?: string[]): Promise<ThoughtWithAuthor | undefined>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getThoughts(options: { limit?: number; tag?: string; authorId?: string; search?: string; exclude?: string } = {}): Promise<ThoughtWithAuthor[]> {
    let query = db
      ...
      .where(eq(thoughts.isPublished, true))
    ...
    if (options.authorId) {
      query = db
        ...
        .from(thoughts)
        .innerJoin(users, eq(thoughts.authorId, users.id))
        .where(eq(thoughts.authorId, options.authorId))
        .orderBy(desc(thoughts.createdAt));
    }
    ...
    if (options.tag) {
      filtered = filtered.filter((t) =>
        t.tags.some((tag) => tag.slug === options.tag)
      );
    }
  ...
  async updateThought(id: string, thought: Partial<InsertThought>, tagNames?: string[]): Promise<ThoughtWithAuthor | undefined> {
  ...
  async getStats(authorId?: string): Promise<Stats> {
    ...
    if (authorId) {
      thoughtsQuery = db.select().from(thoughts).where(eq(thoughts.authorId, authorId));
    }

    const allThoughts = await thoughtsQuery;
    
    const totalThoughts = allThoughts.length;
    const totalWords = allThoughts.reduce((sum, t) => sum + t.wordCount, 0);
    ...
    return { totalThoughts, totalTags, totalWords, totalViews };
  }
}
